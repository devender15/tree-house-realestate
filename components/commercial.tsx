...
const Commercial = () => {
  ...
  useEffect(() => {
    const filtered = properties.filter((property) => property?.category === "commercial")
        ...
        <div className="text-center mb-16">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <p className="text-xl text-slate-600 max-w-3xl mx-auto leading-relaxed font-medium">
              Premium commercial spaces strategically located for your business success and growth opportunities
            </p>
          </motion.div>
        </div>
        ...
        {commercialData.length > 0 ? (
          ...
          >
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-12 border border-slate-200/80 shadow-sm max-w-2xl mx-auto">
              ...
              <h3 className="text-2xl font-bold text-slate-800 mb-4">No Commercial Properties Available</h3>
              <p className="text-slate-600 text-lg leading-relaxed">
                We're currently updating our commercial property portfolio. Please check back soon for exciting new
                opportunities.
              </p>
              <div className="flex items-center justify-center gap-2 mt-6">
                <Award className="w-4 h-4 text-slate-500" />
                <span className="text-sm text-slate-500 font-medium">Premium listings coming soon</span>
              </div>
            </div>
          </motion.div>
        )}
