  ...
  try {
    ...
    if (matchedItem) {
      ...
      if (matchedItem.details) {
        ...
        if (matchedItem.details?.overview?.paragraph1)
          detailsText += matchedItem.details.overview.paragraph1 + "\n\n";
        if (matchedItem.details?.overview?.paragraph2)
          detailsText += matchedItem.details.overview.paragraph2 + "\n\n";
        ...
        return NextResponse.json({
          response: detailsText.trim(),
          source: matchedItem,
        });
      }
      // If it's general info or an array, show as JSON
      return NextResponse.json({
        response: JSON.stringify(matchedItem, null, 2),
        source: matchedItem,
      });
    }
    ...
    if (
      ...
      function extractProperties(type: "residential" | "commercial" | null) {
        ...
        jsonData.forEach((item: any) => {
          ...
          if (
            Array.isArray(item.residentialProperties) &&
            (!type || type === "residential")
          ) {
            item.residentialProperties.forEach((prop: any) => {
              if (
                prop.location &&
                /gurgaon/i.test(prop.location) &&
                prop.name
              ) {
          ...
          if (
            Array.isArray(item.commercialProperties) &&
...
List all ${
        type ? type + " " : ""
      }properties in Gurgaon using ONLY the information below.
